      ...
      it(`injectToStream - useAsync() - ${streamType} stream${disable ? ' - disabled' : ''}`, async () => {
        ...
        if (!disable) {
          ...
          expect(data.content).toMatch(
            partRegex`<script class="react-streaming_initData" type="application/json">{"key":"\\"hello-component-key\\"","value":"Hello, I was lazy.","elementId":"${getUseIdPattern()}"}</script>`,
          )
...
// https://github.com/brillout/react-streaming/issues/58
...
function getUseIdPattern() {
  // The id generated by useId() shouldn't contain `@`, see:
  // https://github.com/brillout/react-streaming/blob/699f8691ff6d206caba395ab51db6bb5b74486f6/src/shared/useSuspense.ts#L187
  return /[^"@]*/
}
